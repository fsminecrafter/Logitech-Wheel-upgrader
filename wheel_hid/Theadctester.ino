...
const char PAIRING_CODE[] = "FSMINEWHEEL123";
...
// Map raw value to -32767..32767 using dynamic min/max
...
void setup() {
  ...
  Wire.begin();
  delay(200);
  Serial.println("PAIRING_REQUEST:" + String(PAIRING_CODE));
}

void loop() {
  // Wait until PC acknowledges pairing
  if (!Serial.available()) {
    delay(50);
    return;
  }
